...
const companyInfoColumns = [
  ...
  { id: 'company_name', accessorKey: 'company_name', header: 'Company Name' , cell: ({ row }) => displayValue(row.getValue("company_name"))},
  {
    ...
    cell: ({ row }) => {
      ...
      const completedFields = nonEmptyFields.length;
  ...
  const DataTable = ({ columns, data, onExport, title }) => {
    ...
    const calculateMissingCounts = (tableData, tableColumns) => {
      ...
      const completedFields = fields.filter(field => 
        tableData.every(row => row[field] !== null && row[field] !== '')
      ).length;
      ...
      tableColumns.forEach(column => {
        if (column.accessorKey !== 'company_name') {
          columnMissing[column.accessorKey] = tableData.filter(row => 
    ...
    const { totalFields, completedFields, totalMissing, columnMissing } = calculateMissingCounts(data, columns);
