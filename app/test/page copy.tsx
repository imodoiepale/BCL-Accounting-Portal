...
export default function GmailManager() {
  ...
  const removeAccount = async () => {
    if (selectedAccount && selectedAccount !== 'all') {
      await revokeAccess(selectedAccount);
    }
  };

  const clearTokenStorage = async (email) => {
    try {
      await supabase
        .from('acc_portal_email_accounts')
        .delete()
